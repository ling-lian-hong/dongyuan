...
class Page
{
	...
	function GetList()
	{
		...
		{
			$pagetxt = '<div class="pages">共<span>'.$this->totalpage.'</span>页<span>'.$this->total.'</span>条记录</div>';
		}
		...
		else
		{
			...
			if($this->totalpage < 10)
			{
				for($i=1; $i <= $this->totalpage; $i++)
				{
					if($this->page == $i)
					{
						$pagetxt .= '<a href="javascript:;" style="border: 1px solid #000000;">&nbsp;&nbsp;'.$i.'&nbsp;&nbsp;</a>';
					}
					else
					{
						//伪静态设置
			...
			else
			{
				...
				for($i=$m; $i<=$b; $i++)
				{
					if($this->page == $i)
					{
						$pagetxt .= '<a href="'.$nowurl.'page='.$i.'" class="current">'.$i.'</a>';
					}
					else
					{
						//伪静态设置
...
public function GetList_Sum()
{
	...
	function GetList_Main($index)
	{
		...
		if($this->total <= $this->pagenum)
		{
			$pagetxt = '<div class="main-pages">共<span>'.$this->totalpage.'</span>页<span>'.$this->total.'</span>条记录</div>';
		}
		...
		else
		{
			...
			if($query_str[0] != '')
			{
				...
				foreach($query_str as $k)
				{
					...
					if(strstr($query_str_arr[0],'main-pages') == '')
					{
						$query_str_arr[0] = isset($query_str_arr[0]) ? $query_str_arr[0] : '';
						$query_str_arr[1] = isset($query_str_arr[1]) ? $query_str_arr[1] : '';
			...
			if($this->page > 1)
			{
				...
				{
					$pagetxt .= ' <a href="'.$nowurl.'page=1'.'#'.$index.'" title="第一页">&lt;&lt;</a> ';
					$pagetxt .= ' <a href="'.$nowurl.'page='.$previous.'#'.$index.'" title="上一页">&lt;</a> ';
				}
			}
			else
			{
			...
			else
			{
				...
				for($i=$m; $i<=$b; $i++)
				{
					if($this->page == $i)
					{
						$pagetxt .= '<a href="'.$nowurl.'pages='.$i.'" class="current">'.$i.'</a>';
					}
					else
					{
						//伪静态设置
						...
						{
							$pagetxt .= '<a href="'.$nowurl.'pages='.$i.'" class="num" title="第 '.$i.' 页">'.$i.'</a>';
						}
					}
				}
				if($this->page < $this->totalpage-3)
			...
			if($this->page < $this->totalpage)
			{
				...
				{
					$pagetxt .= ' <a href="'.$nowurl.'pages='.$next.'" title="下一页">&gt;</a> ';
					$pagetxt .= ' <a href="'.$nowurl.'pages='.$this->totalpage.'" title="最后一页">&gt;&gt;</a> ';
				}
			}
			else
			{
