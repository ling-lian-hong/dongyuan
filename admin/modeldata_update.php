...
<form name="form" id="form" method="post" action="modeldata_save.php" onsubmit="return cfm_modeldata();">
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="formTable">
		...
		<tr>
			...
			<td><input type="text" name="title" id="title" class="input_title" value="<?php echo $row['title']; ?>" /></td>
		</tr>
		<?php
		}
		...
		<tr>
			...
			while($r2 = $dosql->GetArray())
			{
				echo '<span><input type="checkbox" name="flag[]" id="flag[]" value="'.$r2['flag'].'"';

				if(in_array($r2['flag'],$flagarr))
				{
					echo 'checked="checked"';
				}

				echo ' />'.$r2['flagname'].'['.$r2['flag'].']</span>';
			}
			?></td>
		</tr>
		<?php
