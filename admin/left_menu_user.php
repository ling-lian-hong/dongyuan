...
<style type="text/css">
.menubox a.infoattr{top:20px;}
.menubox a.infosrc{top:45px;}
.menubox a.usertype{top:20px;}
.menubox a.adtype{top:46px;}
.menubox a.weblinktype{top:72px;}
.menubox a.goodsinfoattr{top:20px;}
</style>
</head>
<body>
	...
	<div class="viewport">
		<div class="overview">
			...
			else
			{
				//构成管理菜单数组
				$leftMenu01_Str = '';
				$leftMenu01_Arr = array('infoclass'=>'<a href="infoclass.php" target="main">栏目管理</a>',
									'info'=>'<a href="info.php" target="main">单页列表管理</a>',
				                   	'infolist'=>'<a href="infolist.php" target="main">列表信息管理</a>',
					                'message'=>'<a href="message.php" target="main">客户留言管理</a>',
									'soft'=>'<a href="soft.php" target="main">在线客服管理</a>',
									'weblink'=>'<a href="weblink.php" target="main">友情链接管理</a>',	
									'soft'=>'<a href="soft.php" target="main">下载信息管理</a>',
				...
				$leftMenu03_Str = '';
				$leftMenu03_Arr = array('userfavorite'=>'<a href="userfavorite.php" target="main">用户收藏管理</a>',
				...
				$leftMenu04_Str = '';
				$leftMenu04_Arr = array('goodsbrand'=>'<a href="goodsbrand.php" target="main">品牌类型管理</a>',
				...
				if($leftMenu01_Str != '')
				{
					echo '<div class="menubox"><div class="title ton" id="t1" onclick="DisplayMenu(\'leftmenu01\');" title="点击切换显示或隐藏"> 网站内容管理</div><div id="leftmenu01">';
					echo $leftMenu01_Str;
					echo '</div></div><div class="hr_5"></div>';
				}
				if($leftMenu01_Str != '')
				{
					echo '<div class="menubox"><div class="title ton" onclick="DisplayMenu(\'leftmenutu\');" title="点击切换显示或隐藏">图片信息管理</div><div id="leftmenutu" style="display:none">';
					echo GetMgrAjaxType1('#@__infoclass',2);
					echo '</div></div><div class="hr_5"></div>';
				}
				...
				if(@$leftMenu02_Str != '')
				{
					...
					if($leftMenu01_Str != '')
					{
						echo '<div class="menubox"><div class="title ton" onclick="DisplayMenu(\'leftmenu02\');" title="点击切换显示或隐藏"> 网站系统设置</div><div id="leftmenu02" style="display:none">';
					}
					else
					{
						echo '<div class="menubox"><div class="title ton" id="t1" onclick="DisplayMenu(\'leftmenu02\');" title="点击切换显示或隐藏"> 栏目内容管理 </div><div id="leftmenu02">';
					}
				...
				if($leftMenu04_Str != '')
				{
					...
					{
						echo '<div class="menubox"><div class="title ton" id="t1" onclick="DisplayMenu(\'leftmenu04\');" title="点击切换显示或隐藏"> 商品订单管理 </div><div id="leftmenu04">';
					}
				...
				if($leftMenu05_Str != '')
				{
					...
					if($leftMenu01_Str != '' or
					   ...
					   $leftMenu04_Str != '')
					{
						echo '<div class="menubox"><div class="title ton" onclick="DisplayMenu(\'leftmenu05\');" title="点击切换显示或隐藏"> 界面模板管理</div><div id="leftmenu05" style="display:none">';
					}
					else
					{
						echo '<div class="menubox"><div class="title ton" id="t1" onclick="DisplayMenu(\'leftmenu05\');" title="点击切换显示或隐藏"> 界面模板管理</div><div id="leftmenu05">';
					}
				...
				if($leftMenu06_Str != '')
				{
					...
					{
						echo '<div class="menubox"><div class="title ton" id="t1" onclick="DisplayMenu(\'leftmenu06\');" title="点击切换显示或隐藏">帮助与更新 </div><div id="leftmenu06">';
					}
			...
			//显示自定义菜单
			$dosql->Execute("SELECT * FROM `#@__diymenu` WHERE  checkinfo='true' AND parentid=0 ORDER BY orderid ASC");
			$i = 100;
			while($row = $dosql->GetArray())
			{
			?>
			<div class="menubox">
				<div class="title ton" onclick="DisplayMenu('leftmenu<?php echo $i; ?>');" title="点击切换显示或隐藏"><?php echo $row['classname']; ?></div>
			...
			?>
			<!--scrollbar end-->
			<div class="menubox"><div class="title ton" onclick="DisplayMenu('leftmenu02');" title="点击切换显示或隐藏"> 网站系统设置 </div>
				<div id="leftmenu02" style="display: none;">
					<a href="web_config.php" target="main">系统设置</a>
					<a href="admin_update.php?id=<?php echo $_SESSION['adminid'];?>" target="main">登录密码修改</a>
				</div>
			</div>
		</div>
	</div>
...
<div class="copyright">
	© 2013 <a href="http://www.qf580.com" target="_blank">www.qf580.com</a><br />All Rights Reserved.
</div>
<?php
